...
def feedback_form(request):
    """A view that manages the product feedback form"""
    try:
    ...
    if user_orders.count() == 0:
        ...
        messages.error(request,
                       "To give feedback, you must be logged in and have products purchased.")
        return redirect(reverse('index'))

    if request.method == 'POST':
        form = FeedbackForm(request.POST)
        # Get the product name, and what rating the user has given it.
        # Average it with the existing rating.
        if form.is_valid():
